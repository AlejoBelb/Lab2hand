  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .sl-collapse-btn:hover { color: #94a3b8; background: rgba(255,255,255,0.06); }
        ...
        .sl-expand-btn:hover { color: #a78bfa; background: rgba(124,58,237,0.1); }

        .sl-nav { flex: 1; padding: 12px 8px; display: flex; flex-direction: column; gap: 2px; overflow: hidden; }
        .sl-nav-label { font-size: 10px; color: #334155; text-transform: uppercase; letter-spacing: 0.08em; padding: 8px 8px 4px; white-space: nowrap; }
        .sl-nav-link {
          display: flex; align-items: center; gap: 10px;
        ...
        .sl-footer { padding: 12px 8px; border-top: 1px solid rgba(255,255,255,0.06); flex-shrink: 0; }
      ...
      <aside className={`sl-sidebar${sidebarOpen ? '' : ' collapsed'}`}>
        {sidebarOpen ? (
          <div className="sl-logo">
            ...
            <button className="sl-collapse-btn" onClick={() => setSidebarOpen(false)} title="Colapsar panel">
          ...
          <button className="sl-expand-btn" onClick={() => setSidebarOpen(true)} title="Expandir panel">
        ...
        <div className="sl-footer">
          <div className="sl-user-row">
            <div className="sl-avatar">{user?.firstName?.[0]?.toUpperCase() || 'E'}</div>
            {sidebarOpen && (
              <span className="sl-user-name">{user?.firstName || user?.email || 'Estudiante'}</span>
            )}
      ...
      <div className="sl-main">
        ...
        <div className="sl-content"><Outlet /></div>
      </div>
    </div>
  );
}
