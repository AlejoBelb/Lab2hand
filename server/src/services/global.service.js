// server/src/services/global.service.js
// Gestión global de instituciones y usuarios — exclusivo para ADMIN.
...
async function assignAdmin({ userId, institutionId }) {
  ...
  if (user.role === 'ADMIN') {
    const err = new Error('El usuario ya es admin');
    err.statusCode = 400;
    throw err;
  }
  ...
  return prisma.user.update({
    where: { id: userId },
    data: { role: 'ADMIN', isActive: true, pendingApproval: false, institutionId },
    select: {
      id: true, email: true, firstName: true, lastName: true,
      role: true, isActive: true, institutionId: true,
      institution: { select: { id: true, name: true } },
    },
  });
}
...
async function createUserGlobal({ email, password, firstName, lastName, role, institutionId }) {
  ...
  return prisma.user.create({
    data: {
      ...
      role, isActive: true, pendingApproval: false,
